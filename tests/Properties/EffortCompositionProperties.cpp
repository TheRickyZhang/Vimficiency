...
class EffortCompositionGeneratedPropertyTest {
 public:
  void MergeEqualsSequentialAppend(uint32_t seed) {
    RandomGen::seed(seed);
    for (int caseIndex = 0; caseIndex < 300; caseIndex++) {
      SCOPED_TRACE(::testing::Message() << "seed=" << seed << " case=" << caseIndex);
      expectMergeEqualsNaive(randomPhysicalKeys(0, 16), randomPhysicalKeys(0, 16));
    }
  }

  void AssociativityMatchesSequentialAppend(uint32_t seed) {
    RandomGen::seed(seed);
    for (int caseIndex = 0; caseIndex < 300; caseIndex++) {
      SCOPED_TRACE(::testing::Message() << "seed=" << seed << " case=" << caseIndex);
  ...
  void AppendFromMatchesMergeAndSequentialAppend(uint32_t seed) {
    RandomGen::seed(seed);
    for (int caseIndex = 0; caseIndex < 300; caseIndex++) {
      SCOPED_TRACE(::testing::Message() << "seed=" << seed << " case=" << caseIndex);
