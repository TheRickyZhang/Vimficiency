...
### Early Stopping
`optimizeEdit` uses early stopping: once a result is found for a startIndex, further states from that index are skipped (`if (results[s.getStartIndex()].isValid()) continue`). This is safe because A* explores states in cost order, and typed content cost is fixed regardless of path, so the first deletion path found for a position is optimal.
