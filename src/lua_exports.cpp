...
// Global storage
static VimficiencyConfigFFI g_config_ffi; // THIS IS WHERE THE CONFIG LIVES
static Config g_config_internal;

static void sync_config() {
  // We don't break, because additional key config should override on top of defaults.
  switch (g_config_ffi.default_keyboard) {
      case QWERTY:     fill_qwerty(g_config_internal);
      case COLEMAK_DH: fill_colemak_dh(g_config_internal);
      case UNIFORM:    fill_uniform(g_config_internal);
      case NONE: break;
  }
  ...
  // g_config_internal.weights = g_config_ffi.c_scoreWeights; // TODO: good idea to overload this operator= ?
...
extern "C" {
// Note we need to "redefine" (export) these since original declarations are constexpr, and so may be inlined / name mangled
...
const char* vimficiency_analyze(
    ...
    std::vector<Result> res = o.optimizeMovement(
        start_lines,
        end_position,
        keyseq
    );
    ...
    if (res.empty()) {
        ...
        for (const Result& r : res) {
            oss << r.sequence << " " << std::fixed << std::setprecision(3) << r.keyCost << "\n";
        }
    }
