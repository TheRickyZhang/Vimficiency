...
EditFrontierItem frontierItemFromSequence(
    string_view fullSequence,
    double cost,
    CursorPos goalPos) {
  EditSequenceDecomposition decomposition = decomposeEditSequence(fullSequence);
  return EditFrontierItem{
...
vector<EditFrontierItem> rankEditFrontier(
  ...
  params.withMaxEditResultsPerPosition(query.maxCount);
  EditResult editResult = computeEditResultForDiff(query.diff, params, config);
  ...
  span<const Result> starts = editResult.resultsAt(query.cursor.line, query.cursor.col);
  if (!starts.empty()) {
    const CursorPos goalPos = editResult.goalPosAt(query.cursor.line, query.cursor.col);
  ...
  const CursorPos stepGoalPos = editResult.getGoalPos();
