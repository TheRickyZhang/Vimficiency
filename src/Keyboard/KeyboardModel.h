...
static constexpr int HAND_COUNT =
    2; // Unless you're an amputee or an alien; not that those are correlated

#define ENUM_VALUE(name, str) name,
enum class Key : int { VIMFICIENCY_KEYS(ENUM_VALUE) None };

enum class Hand : int8_t { VIMFICIENCY_HANDS(ENUM_VALUE) None };

enum class Finger : int8_t { VIMFICIENCY_FINGERS(ENUM_VALUE) None };
#undef ENUM_VALUE
...
static_assert(FINGER_COUNT == static_cast<uint8_t>(Finger::None),
              "finger counts do not match");
static_assert(HAND_COUNT == static_cast<uint8_t>(Hand::None),
              "hand counts do not match");
