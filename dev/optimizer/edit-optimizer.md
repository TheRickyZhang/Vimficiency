...
### Early Stopping
`optimizeEdit` use early stopping: once a result is found for a startIndex, further states from that index are skipped (`if (results[s.getStartIndex()].isValid()) continue`). Ignoring small differences in running effort patterns, this is optimal.
