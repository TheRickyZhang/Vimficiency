...
Replay / simulate (`:Vimfy sim <alias>`) reconstructs the intermediate state at every step of a captured sequence by driving a hidden probe window through `nvim_feedkeys` and sampling after each token. The probe is our mode-faithful oracle — `:normal` flattens insert/visual, so it isn't usable. Getting the oracle synchronized with Neovim's async input loop is load-bearing; see `dev/lua/replay-precompute.md` for the drain strategy (synchronous `nx` for pure Normal-mode motions, coroutine-yield for modal-entering tokens) and the per-token telemetry that the `D` debug keybind surfaces.
